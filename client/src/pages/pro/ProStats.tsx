    ...
    <ProLayout title="Performances" back="/derm/dashboard">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <KPI icon={<TrendingUp className="w-4 h-4" />} accent="#0369A1" label="Glow Score moyen" value={`${data.avgGlowScore}/100`} testid="kpi-avg-score" />
          <ProCard className="p-4">
            ...
            >
              <Calendar className="w-4 h-4" style={{ color: "#0369A1" }} />
            </div>
            <p className="text-[11px] font-extrabold uppercase tracking-widest mb-2" style={{ color: "#475569" }}>Statut patients</p>
            <div className="flex items-center gap-2.5 text-xs font-extrabold flex-wrap">
              ...
              <span className="flex items-center gap-1" style={{ color: "#64748B" }} title="Résolu">
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          <Section title="Top 5 problèmes de peau" icon={<BarChart3 className="w-4 h-4" />} accent={NAVY}>
            {data.topConditions.length === 0 && <p className="text-xs" style={{ color: "#475569" }}>Aucune donnée encore.</p>}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="truncate flex-1 font-bold" style={{ color: "#475569" }}>{c.name}</span>
                    <span className="font-extrabold" style={{ color: NAVY }}>{c.count}</span>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "#F1F5F9" }}>
                    <div className="h-full rounded-full" style={{ width: `${(c.count / max) * 100}%`, background: "linear-gradient(90deg, #7c3aed, #0369A1)" }} />
                  </div>
                </motion.div>
              );
            })}
          ...
          <Section title="Top 5 produits recommandés" icon={<ShoppingBag className="w-4 h-4" />} accent={GREEN}>
            {data.topProducts.length === 0 && <p className="text-xs" style={{ color: "#475569" }}>Aucune donnée encore.</p>}
            {data.topProducts.map((p) => {
              const max = data.topProducts[0]?.count || 1;
              return (
                <div key={p.name} className="mb-3 last:mb-0">
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="truncate flex-1 font-bold" style={{ color: "#475569" }}>{p.name}</span>
        ...
        <Section title="Consultations par mois" icon={<Calendar className="w-4 h-4" />} accent="#0369A1">
          {data.monthly.length === 0 && <p className="text-xs" style={{ color: "#475569" }}>Aucune donnée encore.</p>}
              ...
              {data.monthly.map((m) => {
                ...
                return (
                  <div key={m.month} className="flex-1 flex flex-col items-center gap-1.5 group">
                    <span className="text-[10px] font-extrabold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "#0369A1" }}>
                      {m.count}
                    </span>
                    <div
                      className="w-full rounded-t-md transition-all hover:opacity-80 min-h-[4px]"
                      style={{ height: `${(m.count / max) * 100}%`, background: "linear-gradient(to top, #7c3aed, #0369A1)" }}
                    />
                    <span className="text-[10px] font-medium" style={{ color: "#64748B" }}>{m.month.slice(5)}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          <Section title="Répartition par phototype" icon={<Users className="w-4 h-4" />} accent="#0369A1">
            {(!phototypeDist || phototypeDist.length === 0) && <p className="text-xs" style={{ color: "#475569" }}>Aucune donnée encore.</p>}
            {phototypeDist.map((p: any) => {
              const max = phototypeDist[0]?.count || 1;
              return (
                <div key={p.name} className="mb-3 last:mb-0">
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="font-bold" style={{ color: "#475569" }}>Fitzpatrick {p.name}</span>
                    <span className="font-extrabold" style={{ color: "#0369A1" }}>{p.count}</span>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "#F1F5F9" }}>
                    <div className="h-full rounded-full" style={{ width: `${(p.count / max) * 100}%`, background: "linear-gradient(90deg,#7c3aed,#0891B2)" }} />
                  </div>
                </div>
              );
            })}
          ...
          <Section title="Consultations en ligne" icon={<TrendingUp className="w-4 h-4" />} accent={GREEN}>
            <div className="flex gap-4 mb-4">
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: "#475569" }}>Consultations</p>
              </div>
              <div>
                <p className="text-2xl font-extrabold" style={{ color: "#6ee7b7" }}>{onlineRevenue.toLocaleString("fr-FR")}</p>
                <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: "#475569" }}>FCFA reçus</p>
              </div>
            </div>
                ...
                {onlineRevenueMonthly.slice(-6).map((m: any) => {
                  ...
                  return (
                    <div key={m.month} className="flex-1 flex flex-col items-center gap-1 group">
                      ...
                      <span className="text-[9px]" style={{ color: "#64748B" }}>{m.month.slice(5)}</span>
                    </div>
                  );
                })}
              </div>
...
function KPI({ icon, accent, label, value, testid }: any) {
  return (
    <ProCard className="p-4" testid={testid}>
      ...
      <p className="text-[11px] font-extrabold uppercase tracking-widest mt-0.5" style={{ color: "#475569" }}>{label}</p>
    </ProCard>
  );
}
