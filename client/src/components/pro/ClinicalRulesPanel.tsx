...
const META: Record<string, { color: string; bg: string; tag: string }> = {
  urgent: { color: "#dc2626", bg: "rgba(248,113,113,0.1)", tag: "URGENT" },
          ...
          return (
            <div key={r.id} className="rounded-xl p-2.5" style={{ background: m.bg, border: `1px solid ${m.color}33` }}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[11.5px] font-extrabold" style={{ color: "#0F172A" }}>{r.label}</span>
              </div>
              <p className="text-[11px] leading-relaxed" style={{ color: "#475569" }}>→ {r.action}</p>
            </div>
          );
        })}
      </div>
